...
export async function cleanTextWithAI(
  ...
  const MAX_INPUT_TOKENS = 180000; // Leave room for system prompt and output
  const MAX_OUTPUT_TOKENS = 16384; // Maximum output tokens
  ...
  if (estimatedTokens < MAX_INPUT_TOKENS) {
    ...
    if (onProgress) {
      let simulatedLine = 0;
      const lineIncrement = Math.max(1, Math.floor(totalLines / 40)); // Update ~40 times
    ...
    try {
      const message = await client.messages.create({
        ...
        system: TEXT_CLEANING_SYSTEM_PROMPT,
        messages: [
          {
            role: 'user',
      ...
      if (content.type === 'text') {
        ...
        if (onProgress) {
          onProgress(1, 1, totalLines, totalLines);
        }
  ...
  // Use a more conservative chunk size to ensure we don't exceed token limits
  const MAX_CHARS_PER_CHUNK = MAX_INPUT_TOKENS * 3; // ~3 chars per token
  ...
  for (let i = 0; i < chunks.length; i++) {
    ...
    // Calculate which line we're starting from based on character position
    const textBeforeChunk = text.substring(0, processedChars);
    const linesBeforeChunk = textBeforeChunk.split('\n').length - 1;
    const chunkLines = chunk.split('\n').length;
    const startLine = linesBeforeChunk + 1; // 1-indexed for display
    
    // Update progress: show we're starting this chunk
    if (onProgress) {
      onProgress(i, chunks.length, startLine, totalLines);
    }
    
    // Simulate progress during API call by updating line position
    let progressInterval: NodeJS.Timeout | null = null;
    if (onProgress) {
      let simulatedLine = startLine;
      const lineIncrement = Math.max(1, Math.floor(chunkLines / 20)); // Update ~20 times per chunk
    ...
    try {
      const message = await client.messages.create({
        model,
        max_tokens: MAX_OUTPUT_TOKENS,
        system: TEXT_CLEANING_SYSTEM_PROMPT + '\n\nIMPORTANT: This is part ' + (i + 1) + ' of ' + chunks.length + ' chunks. Clean this section completely, preserving all clause numbers and content. Do not add headers or footers indicating this is a chunk.',
        messages: [
          {
            role: 'user',
            content: `Clean the following contract text section. Fix all PDF-to-text conversion issues while preserving legal meaning and clause numbering exactly:\n\n${chunk}`
          }
        ]
      });
      ...
      const content = message.content[0];
      if (content.type === 'text') {
        cleanedChunks.push(content.text.trim());
        processedChars += chunk.length;
        
        // Update progress: show we've completed this chunk
      ...
      if (error.message?.includes('401') || error.message?.includes('authentication')) {
        ...
        for (let j = 0; j < subChunks.length; j++) {
          ...
          const subMessage = await client.messages.create({
            model,
            max_tokens: MAX_OUTPUT_TOKENS,
            system: TEXT_CLEANING_SYSTEM_PROMPT,
            messages: [
          ...
          const subContent = subMessage.content[0];
          if (subContent.type === 'text') {
            cleanedChunks.push(subContent.text.trim());
  ...
  // Merge cleaned chunks with proper spacing
